function [wcohere, ax_0, ay_0,scale] = calcCMC_original(x,y,N,n,f)
    % n sample points
    ...
    ax_0=0; ay_0=0; axy_0=0;
    X=cell(1,N);Y=cell(1,N);sX=cell(1,N);sY=cell(1,N);Wxy=cell(1,N);sWxy=cell(1,N);
    % trials
    % Rsqvalue=cell(1,N);
    fprintf("N=%d", N);
    for kn=1:N
        ...
        [Y{1,kn},period,scale] = wavelet(y(:,kn),dt,Args.Pad,Args.Dj,Args.S0,Args.J1,Args.Mother,Args.Cycles);
        %Smooth X and Y before truncating!  (minimize coi)
        nx=size(x(:,kn),1);
        ny=size(y(:,kn),1);
        sinv=1./(scale');
        sX{1,kn}=smoothwavelet(sinv(:,ones(1,nx)).*(abs(X{1,kn}).^2),dt,period,Args.Dj,scale);
        sY{1,kn}=smoothwavelet(sinv(:,ones(1,ny)).*(abs(Y{1,kn}).^2),dt,period,Args.Dj,scale);
        % -------- Cross wavelet -------
        Wxy{1,kn}=X{1,kn}.*conj(Y{1,kn});
        % ----------------------- Wavelet coherence ---------------------------------
        sWxy{1,kn}=smoothwavelet(sinv(:,ones(1,n)).*Wxy{1,kn},dt,period,Args.Dj,scale);
        ax_0=ax_0+sX{1,kn};
        ay_0=ay_0+sY{1,kn};
        axy_0=axy_0+sWxy{1,kn};
        if kn == 100
            break;
        end
    end
